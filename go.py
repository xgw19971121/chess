...
def CalcScore(board,y=3,x=3):
    ...
    for n, player in enumerate([1+step%2, 2-step%2]): # n=0:mine,n=1:oppo 
        ...
        for i in range(int(np.max(components))):
            ...
            if qi < 4: score += (2*n-1) * (1.6-0.5*qi) * (1+count/(9+n*16))
    ...
    corner = 0.02*MapL + 3.13 - step/2000
    score += 0.03/(min(abs(y-corner),abs(y-MapL-1+corner))+min(abs(x-corner),abs(x-MapL-1+corner)))
